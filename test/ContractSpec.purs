    ...
    it "static pages contain only the pinned inline scripts (ADR-000)" do
      for_ staticRoutes \route ->
        for_ allLangs \lang -> do
          ...
          html `StrAssert.shouldContain` "if(!history.state)history.replaceState({__ajax:true},'',location.href);window.addEventListener('ajax:merged',function(){var m=document.getElementById('content');if(m&&m.dataset.pageTitle)document.title=m.dataset.pageTitle});window.addEventListener('popstate',function(e){if(e.state&&e.state.__ajax){e.stopImmediatePropagation();fetch(window.location.href,{headers:{'X-Alpine-Request':'true'}}).then(function(r){return r.text()}).then(function(h){var d=new DOMParser().parseFromString(h,'text/html');var nc=d.getElementById('content'),cc=document.getElementById('content');if(nc&&cc){cc.replaceWith(nc);if(nc.dataset.pageTitle)document.title=nc.dataset.pageTitle}var nn=d.getElementById('nav'),cn=document.getElementById('nav');if(nn&&cn)cn.replaceWith(nn);if(window.Alpine)window.Alpine.initTree(document.body)})}},true);"
