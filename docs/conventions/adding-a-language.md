...
5. **No JS edits** — do **not** edit `App.Layout.Scripts.pageSyncScript`. Href/OG alternate sync is driven by `pageSyncAttrs` (`data-page-href-*` / `data-page-og-alts` from `allLangs`) and the generic `pageHref*` / `pageOgAlts` iterators in the TitleSync IIFE.

Chrome copy (nav home, etc.) comes from the dictionary (`d.nav.home`, …), not hardcoded shell helpers.

Verify with `make test` (ContractSpec pins one `data-page-href-*` per `allLangs` and that the TitleSync script mentions `pageHref` / `pageOgAlts`).
