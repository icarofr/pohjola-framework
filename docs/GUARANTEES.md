...
|---|-----------|-------------|-------------|
| 1 | No partial functions — `fromJust`, `Data.Maybe.Unsafe`, `Data.Array.Unsafe`, `Data.String.CodePoint.Unsafe` cannot appear in `src/` | `Policy.Contract` → `make gate` | `make gate` |
| 2 | No `unsafeCoerce` / `unsafePerformEffect` / `unsafePartial` | `Policy.Contract` → `make gate` | `make gate` |
| 3 | No unapproved FFI — `foreign import` in `src/` fails the build unless the module is allowlisted (default: none) | `Policy.Contract` (`ffiAllowlist`) → `make gate` | `make gate` |
...
| 16 | Rendered HTML never contains unescaped `<`/`>` from `text` | Property tests (quickcheck) | `make test` |
