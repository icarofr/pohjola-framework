...
-- | The template dispatch alone, no shell wrap — shared by renderPage (full
-- | page) and the Datastar SSE-patch path (App.Main), which both need the
-- | exact same body content, just wrapped differently.
renderPageBody :: Lang -> Route -> PageTemplate -> Html
renderPageBody lang route = case _ of
  Landing slots ->
...
renderPage lang route status template =
  DatastarShell.dsSitePage lang route status (renderPageBody lang route template)
