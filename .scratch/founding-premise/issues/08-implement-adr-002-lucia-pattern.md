...
8. [ ] **Not done — real, open gap, not a formality.** `ADR-005` (CSRF) is still "Accepted — implementation pending." `GUARANTEES.md` now states plainly: don't wire `requireAuth` into a mutating route (a form submission) before CSRF lands — read-only protected pages are lower risk, but nothing here should be treated as safe against the class of attack CSRF prevents yet.
